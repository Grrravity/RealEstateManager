...
):Parcelable
{
    ...
    fun fromContentValues(values: ContentValues):Property{
        ...
        if (values.containsKey("EntryDate")) property.dateOfEntry       = values.getAsString("EntryDate")
        if (values.containsKey("dateOfSale"))
            if (values.getAsString("dateOfSale") == "") property.dateOfSale = null
            else property.dateOfSale                                    = values.getAsString("dateOfSale")
        if (values.containsKey("agent")) property.realtor               = values.getAsString("agent")
        if (values.containsKey("bathNumber")) property.numOfBath        = values.getAsInteger("bathNumber")
        if (values.containsKey("bedNumber")) property.numOfBed          = values.getAsInteger("bedNumber")
        if (values.containsKey("address")) property.address?.address    = values.getAsString("address")
        if (values.containsKey("apartmentNumber"))
            if (values.getAsInteger("apartmentNumber") == 0) property.address?.apartmentNumber = null
            else property.address?.apartmentNumber                      = values.getAsInteger("apartmentNumber")
        ...
        if (values.containsKey("lat"))
            if (values.getAsDouble("lat") == 0.0) property.address?.lat = null
            else property.address?.lat                                  = values.getAsDouble("lat")
        if (values.containsKey("lng"))
            if (values.getAsDouble("lng") == 0.0) property.address?.lng = null
            else property.address?.lng                                  = values.getAsDouble("lng")
        return property
    }
}